    ...
    <div>
      ...
      <div>
        {appointments.map((item, index) => (
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-p' key={index}>
            <div>
              <img className='w-32 bg-indigo-50 rounded-md' src={item.docData.image} alt="" />
            </div>
            <div className='flex-1 text-zinc-600'>
              <p className='text-blue-600 font-semibold'>{item.docData.name}</p>
              <p>{item.docData.speciality}</p>
              <p className='text-zinc-700 font-medium mt-1'>Address</p>
              <p className='text-xs'>{item.docData.address.line1}</p>
              <p className='text-xs'>{item.docData.address.line2}</p>
              <p className='text-xs mt-1 text-red-600 font-medium'><span className='text-sm text-green-600 font-medium'>Date & Time: </span>{slotDateFormat(item.slotDate)}| {item.slotTime}</p>
            </div>
            <div></div>
            <div className='flex flex-col gap-2 justify-end'>
              {!item.cancelled && item.payment && !item.isCompleted && <button className='sm:min-w-48 py-2 border rounded text-gray-500 bg-green-600'>Paid</button>}
              ...
              {item.isCompleted && <button className='sm:min-w-48 py-2 border border-green-500 text-green-500 rounded font-semibold'>Completed</button>}
            </div>
          </div>
        ))}
      </div>
